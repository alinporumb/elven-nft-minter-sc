...
pub trait Setup: storage::Storage + operations::Operations {
    ...
    match properties_option {
      Some(value) => {
        for token_propery in value.into_iter() {
          match token_propery {
            NFTProperties::CanFreeze => {
              properties.can_freeze = true;
            }
            NFTProperties::CanWipe => {
              properties.can_wipe = true;
            }
            NFTProperties::CanPause => {
              properties.can_pause = true;
            }
            NFTProperties::CanTransferCreateRole => {
              properties.can_transfer_create_role = true;
            }
            NFTProperties::CanChangeOwner => {
              properties.can_change_owner = true;
            }
            NFTProperties::CanUpgrade => {
              properties.can_upgrade = true;
            }
  ...
  fn set_drop(
    ...
    let tokens_limit = tokens_limit_per_address_per_drop.into_option().unwrap_or_default();
    let tokens_limit_total = self.tokens_limit_per_address_total().get();

    require!(
      tokens_limit <= tokens_limit_total,
      "The tokens limit per address per drop should be smaller or equal to the total limit of tokens per address!"
    );
    ...
    self.amount_of_tokens_per_drop().set(&amount_of_tokens_per_drop);

    if self.last_drop().is_empty() {
      self.last_drop().set(1);
    } else {
      self.last_drop().update(|sum| {
        *sum += 1;
      });
    }
  ...
  fn giveaway(&self, addresses: ManagedVec<ManagedAddress>, amount_of_tokens_per_address: u32) {
    ...
    require!(roles.has_role(&EsdtLocalRole::NftCreate), "NFTCreate role not set!");

    require!(
      self.get_current_left_tokens_amount() >=
        amount_of_tokens_per_address * (addresses.len() as u32),
